    ...
    <div className={cn("relative", sizeClasses[size], className)}>
      <div className="absolute h-full w-full">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className="bg-muted-foreground absolute animate-[spinner-fade_1.2s_linear_infinite] rounded-full"
            style={{
              top: "0",
              left: "50%",
              marginLeft:
...
function PulseLoader({
  ...
  return (
    <div className={cn("relative", sizeClasses[size], className)}>
      <div className="border-muted-foreground absolute inset-0 animate-[thin-pulse_1.5s_ease-in-out_infinite] rounded-full border-2" />
      <span className="sr-only">Loading</span>
    </div>
  );
}
...
function PulseDotLoader({
  ...
  return (
    <div
      className={cn(
        "bg-muted-foreground animate-[pulse-dot_1.2s_ease-in-out_infinite] rounded-full",
        sizeClasses[size],
        className,
      )}
    >
    ...
    >
      {[...Array(5)].map((_, i) => (
        <div
          key={i}
          className={cn(
            "bg-muted-foreground animate-[wave_1s_ease-in-out_infinite] rounded-full",
            barWidths[size],
          )}
          style={{
    ...
    <div className={cn("flex", containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div
          key={i}
          className={cn(
            "bg-muted-foreground h-full animate-[wave-bars_1.2s_ease-in-out_infinite]",
            barWidths[size],
          )}
          style={{
...
function TerminalLoader({
  ...
  return (
    ...
    >
      <span className={cn("text-muted-foreground font-mono", textSizes[size])}>
        {">"}
      </span>
      <div
        className={cn(
          "bg-muted-foreground animate-[blink_1s_step-end_infinite]",
          cursorSizes[size],
        )}
      />
      <span className="sr-only">Loading</span>
  ...
  return (
    <div className={cn("inline-flex items-center", className)}>
      ...
      <span className="inline-flex">
        <span className="text-muted-foreground animate-[loading-dots_1.4s_infinite_0.2s]">
          .
        </span>
        <span className="text-muted-foreground animate-[loading-dots_1.4s_infinite_0.4s]">
          .
        </span>
        <span className="text-muted-foreground animate-[loading-dots_1.4s_infinite_0.6s]">
          .
        </span>
      </span>
    </div>
